...
import (
	...
	"go-chi-ent-example/ent/predicate"
...
type UserMutation struct {
	...
	clearedcars    bool
	_Groups        map[int]struct{}
	removed_Groups map[int]struct{}
	cleared_Groups bool
	done           bool
...
func (m *UserMutation) AddGroupIDs(ids ...int) {
	if m._Groups == nil {
		m._Groups = make(map[int]struct{})
	}
	for i := range ids {
		m._Groups[ids[i]] = struct{}{}
	}
}

// ClearGroups clears the "Groups" edge to the Group entity.
func (m *UserMutation) ClearGroups() {
	m.cleared_Groups = true
}

// GroupsCleared reports if the "Groups" edge to the Group entity was cleared.
func (m *UserMutation) GroupsCleared() bool {
	return m.cleared_Groups
}

// RemoveGroupIDs removes the "Groups" edge to the Group entity by IDs.
func (m *UserMutation) RemoveGroupIDs(ids ...int) {
	if m.removed_Groups == nil {
		m.removed_Groups = make(map[int]struct{})
	}
	for i := range ids {
		delete(m._Groups, ids[i])
		m.removed_Groups[ids[i]] = struct{}{}
	}
}

// RemovedGroups returns the removed IDs of the "Groups" edge to the Group entity.
func (m *UserMutation) RemovedGroupsIDs() (ids []int) {
	for id := range m.removed_Groups {
		ids = append(ids, id)
	}
	return
}
...
func (m *UserMutation) ResetGroups() {
	m._Groups = nil
	m.cleared_Groups = false
	m.removed_Groups = nil
}
...
func (m *UserMutation) AddedIDs(name string) []ent.Value {
	...
	case user.EdgeGroups:
		ids := make([]ent.Value, 0, len(m._Groups))
		for id := range m._Groups {
			ids = append(ids, id)
		}
		return ids
...
func (m *UserMutation) RemovedIDs(name string) []ent.Value {
	...
	case user.EdgeCars:
		...
		return ids
	case user.EdgeGroups:
		ids := make([]ent.Value, 0, len(m.removed_Groups))
		for id := range m.removed_Groups {
			ids = append(ids, id)
		}
		return ids
